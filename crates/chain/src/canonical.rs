...
///
/// Use [`txs`](Canonical::txs) to iterate in canonical order, or
/// [`txs_in_topological_order`](CanonicalView::txs_in_topological_order) for an ordering where
/// every transaction appears after the transactions it spends from.
...
impl<A, P: Clone> Canonical<A, P> {
    ...
    ///
    /// For [`CanonicalView`], an ordering where every transaction appears after the transactions
    /// it spends from is available via
    /// [`txs_in_topological_order`](CanonicalView::txs_in_topological_order).
    ///
    /// # Example
    ///
    /// ```
...
impl<A: Anchor> CanonicalView<A> {
    ...
    /// - among transactions whose spending constraints are already satisfied, ties are broken by
    ///   chain position (confirmed transactions by block height, then unconfirmed) and then by txid
    ///
    /// The result is therefore fully deterministic. The ordering is computed with Kahn's algorithm.
    fn topological_sort(&self) -> Vec<Txid> {
        ...
        let mut sorted = Vec::with_capacity(self.txs.len());
