...
impl<'g, A: Anchor, B: ToBlockHash> ChainTask<B> for CanonicalTask<'g, A, B> {
    ...
    fn poll(&mut self) -> TaskProgress {
        match self.current_stage {
            ...
            CanonicalStage::AnchoredTxs => {
                // We query block data lazily, one frontier tx at a time, rather than batching all
                // anchor heights up front. This is deliberate: anchored txs are processed
                // latest-first, and marking one canonical sweeps all its ancestors transitively
                // (see `mark_canonical`) without any further queries. So the number of queries is
                // bounded by the *frontier* of each confirmed subgraph, not the total tx count.
                // Eager batching would over-query heights for ancestors that transitivity covers
                // for free.
                //
                // The trade-off: a driver where each query is a network round-trip (e.g. a future
                // Bitcoin Core RPC driver) sees one serial round-trip per frontier tx, instead of
                // a hashmap lookup.
                //
                // Intended optimization (left for the follow-up that adds such a driver): before
                // querying, find the *anchored leaves* by traversing the spend graph — the anchored
                // txs that are not ancestors of any other anchored tx. Verifying a leaf sweeps its
                // whole ancestor cone transitively, so the leaf set is the minimal set of heights
                // we can ever need, computed up front with no queries. This gives:
                //   - a single batched query for all leaf heights instead of K serial round-trips;
                //   - a known query upper bound before any I/O (so a driver can size its batch,
                //     report progress, and bound its work).
                // Two caveats: (1) a leaf whose anchor fails verification (stale anchor / reorg)
                // does not sweep its ancestors, so its parents become the next frontier — making
                // this level-by-level, with depth 1 in the common no-reorg case; (2) this must only
                // change *when* heights are fetched, not the order `mark_canonical` runs, since
                // conflict resolution depends on the latest-first / assume-canonical mark order.
...
mod tests {
    ...
    fn test_mtp_with_header_chain() {
        ...
        // `prev_mtp` for an output of a tx confirmed at height 5: the MTP of the block
        // *preceding* the confirmation height (BIP-68), i.e. MTP at height 4 = median of
        // timestamps at heights 0..=4.
        // Timestamps: 100, 200, 300, 400, 500 → sorted, median = ts[len/2] = ts[2] = 300.
        // (Was 400 when the buggy code medianed heights 0..=5 at the confirmation height itself.)
        let txo = view
            .txout(bitcoin::OutPoint::new(txid, 0))
            .expect("tx has an output at vout 0");
        assert_eq!(txo.prev_mtp, Some(300));
