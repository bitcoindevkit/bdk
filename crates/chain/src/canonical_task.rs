use crate::canonical_iter::{CanonicalReason, CanonicalizationParams, ObservedIn};
...
mod tests {
    ...
    use super::*;
    use crate::{local_chain::LocalChain, ChainOracle};
    use bitcoin::{hashes::Hash, BlockHash, TxIn, TxOut};

    fn handle_canonicalization_request<A: Anchor>(
        chain: LocalChain,
    ...
    fn test_canonicalization_task_sans_io() {
        ...
        if let Some(request) = initial_request {
            let response = handle_canonicalization_request(chain.clone(), &request).unwrap();
