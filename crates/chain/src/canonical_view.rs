...
impl<A: Anchor> CanonicalView<A> {
    ...
    pub fn extract_subgraph(&mut self, txids: impl Iterator<Item = Txid>) -> Self {
        use crate::collections::hash_map::Entry;

        let mut to_vist = txids.collect::<Vec<Txid>>();

        let mut spends = HashMap::<OutPoint, Txid>::new();
        let mut txs = HashMap::<Txid, (Arc<Transaction>, ChainPosition<A>)>::new();

        while let Some(txid) = to_vist.pop() {
