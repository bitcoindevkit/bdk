...
fn test_verify_transaction() {
    ...
    // Insert the spending child alone, neither verify because we don't have prevouts
    let mut graph = TxGraph::<ConfirmationHeightAnchor>::default();
    let _ = graph.insert_tx(spending.clone());
    assert!(matches!(
        graph.verify_tx(&spending).unwrap_err(),
        VerifyTxError::Script(Error::UnknownSpentOutput(_))
    ));
    assert!(matches!(
        graph.verify_tx(&spent).unwrap_err(),
        VerifyTxError::Script(Error::UnknownSpentOutput(_))
    ));
    // Now insert the spent parent, spending tx passes verification
    let _ = graph.insert_tx(spent);
    assert!(graph.verify_tx(&spending).is_ok());
    // Mutate one of the inputs, tx no longer verifies
    let mut tx = spending.clone();
    tx.input[0].script_sig = ScriptBuf::from_bytes([42].to_vec());
    assert!(matches!(
        graph.verify_tx(&tx).unwrap_err(),
        VerifyTxError::Script(Error::BitcoinConsensus(_))
    ));
}
