...
use bdk_chain::{
    ...
    BlockId, ChainOracle, ObservedAs,
};
use bitcoin::{
...
fn test_chain_spends() {
    ...
    let _ = [95, 98]
        ...
        .for_each(|(ht, tx)| {
            let block_hash = local_chain
                .get_block_in_best_chain(*ht)
                .unwrap()
    ...
    assert_eq!(
        graph
            .get_chain_spend(&local_chain, OutPoint::new(tx_0.txid(), 0))
            .unwrap(),
        (
            ObservedAs::Confirmed(&BlockId {
                height: 98,
                hash: local_chain
                    .get_block_in_best_chain(98)
                    .unwrap()
                    .expect("block expected")
            }),
            tx_1.txid()
    ...
    assert_eq!(
        graph
            .get_chain_position(&local_chain, tx_0.txid())
            .expect("position expected"),
        ObservedAs::Confirmed(&BlockId {
            height: 95,
            hash: local_chain
                .get_block_in_best_chain(95)
                .unwrap()
                .expect("block expected")
        })
    );
