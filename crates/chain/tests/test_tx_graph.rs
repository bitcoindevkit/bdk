...
/// A helper structure to construct multiple [`TxGraph`] scenarios, used in
/// `test_txs_in_topological_order`.
struct Scenario<'a> {
...
/// A helper method to check whether the given txids are in topological order.
fn is_ordered_topologically(txs: &[Txid], tx_graph: &TxGraph<BlockId>) -> bool {
...
#[test]
fn test_txs_in_topological_order() {
    // chain
