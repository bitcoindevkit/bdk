...
impl<E: ElectrumApi> BdkElectrumClient<E> {
    ...
    /// # let client = electrum_client::Client::new("ssl://electrum.blockstream.info:50002")?;
    /// # let bdk_client = BdkElectrumClient::new(client);
    /// let spk = electrum_client::bitcoin::ScriptBuf::new_op_return(&[0x00; 20]);
    /// let request = FullScanRequest::<&str>::builder()
    ///     .spks_for_keychain("alice", [(0u32, spk)])
    ...
    ///     .build();
    ///
    /// let response = bdk_client.full_scan(request, 10, 50, true)?;
    /// # Ok::<_, electrum_client::Error>(())
    /// ```
    ///
    ...
    ///     .build();
    ///
    /// let response = bdk_client.sync(request, 50, true)?;
    /// # Ok::<_, electrum_client::Error>(())
    /// ```
    ///
