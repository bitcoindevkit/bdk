...
/// Default coin selection algorithm used by [`TxBuilder`](super::tx_builder::TxBuilder) if not
/// overridden. No fallback [`SingleRandomDraw`] coin selection algorithm is provided.
#[cfg(not(feature = "std"))]
...
impl BranchAndBoundCoinSelection<()> {
    /// Create new instance with a target `size_of_change` and no fallback_algorithm.
    pub fn new_no_fallback(size_of_change: u64) -> Self {
        Self {
            size_of_change,
            fallback_algorithm: None,
        }
    ...
    #[allow(dead_code)] // not really dead code
    fn coin_select(
        &mut self,
        required_utxos: Vec<WeightedUtxo>,
        optional_utxos: Vec<WeightedUtxo>,
...
impl<Cs> BranchAndBoundCoinSelection<Cs> {
    fn bnb_coin_select(
        &mut self,
        required_utxos: &[WeightedUtxo],
        optional_utxos: &[WeightedUtxo],
        fee_rate: FeeRate,
        target_amount: u64,
        drain_script: &Script,
    ) -> Result<CoinSelectionResult, Error> {
...
pub struct SingleRandomDraw<R>(pub R);
...
mod test {
    ...
    fn test_bnb_function_no_exact_match() {
        ...
        let mut bnb_no_fallback = BranchAndBoundCoinSelection::new_no_fallback(8 + 1 + 22);
        let result =
    ...
    fn test_bnb_function_tries_exceeded() {
        ...
        let result = BranchAndBoundCoinSelection::default().bnb(
            vec![],
            utxos,
            0,
            curr_available_value,
