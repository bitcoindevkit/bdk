...
use alloc::sync::Arc;
use bdk_bitcoind_client::{corepc_types, Client};
...
pub use bdk_bitcoind_client;

/// The [`Emitter`] is used to emit data sourced from [`bdk_bitcoind_client::Client`].
///
...
impl Emitter {
    ...
    #[cfg(feature = "std")]
    pub fn mempool(&mut self) -> Result<MempoolEvent, bdk_bitcoind_client::Error> {
    ...
    pub fn mempool_at(
        &mut self,
        sync_time: u64,
    ) -> Result<MempoolEvent, bdk_bitcoind_client::Error> {
        ...
        let mut mempool_event = MempoolEvent {
            update: rpc_mempool
                .into_iter()
                .filter_map(|txid| -> Option<Result<_, bdk_bitcoind_client::Error>> {
                    let tx = match self.mempool_snapshot.get(&txid) {
                        Some(tx) => tx.clone(),
                        None => match client.get_raw_transaction(&txid) {
                            Ok(tx) => {
                                let tx = Arc::new(tx);
    ...
    pub fn next_block(&mut self) -> Result<Option<BlockEvent<Block>>, bdk_bitcoind_client::Error> {
...
enum PollResponse {
    Block(corepc_types::model::GetBlockHeaderVerbose),
    NoMoreBlocks,
    /// Fetched block is not in the best chain.
    BlockNotInBestChain,
    AgreementFound(
        corepc_types::model::GetBlockHeaderVerbose,
...
fn poll_once(emitter: &Emitter) -> Result<PollResponse, bdk_bitcoind_client::Error> {
    ...
    if let Some(last_res) = &emitter.last_block {
        ...
        let res = client.get_block_header_verbose(&next_hash)?;
    ...
    for cp in emitter.last_cp.iter() {
        let res = match client.get_block_header_verbose(&cp.hash()) {
...
fn poll(
    emitter: &mut Emitter,
) -> Result<Option<(CheckPoint<BlockHash>, Block)>, bdk_bitcoind_client::Error> {
    loop {
        match poll_once(emitter)? {
            PollResponse::Block(res) => {
                let height = res.height;
                let hash = res.hash;
                let item = emitter.client.get_block(&hash)?;
