...
fn load_recovers_wallet() {
    ...
    let (desc, chng) = get_test_tr_single_sig_xprv_with_change();

    // create new wallet
    let wallet_spk_index = {
        let db = bdk_file_store::Store::create_new(DB_MAGIC, &file_path).expect("must create db");
        let mut wallet = Wallet::new(desc, chng, db, Network::Testnet).expect("must init wallet");
    ...
    // recover wallet
    {
        ...
        let wallet = Wallet::load(desc, chng, db).expect("must recover wallet");
    ...
    {
        let db = bdk_file_store::Store::open(DB_MAGIC, &file_path).expect("must recover db");
        let result = Wallet::new(desc, chng, db, Network::Testnet);
...
fn new_or_load() {
    ...
    let temp_dir = tempfile::tempdir().expect("must create tempdir");
    let file_path = temp_dir.path().join("store.db");
    let (desc, chng) = get_test_wpkh_with_change();
    ...
    let wallet_keychains = {
        ...
        let wallet =
            Wallet::new_or_load(desc, chng, db, Network::Testnet).expect("must init wallet");
    ...
    // wrong network
    {
        ...
        let err = Wallet::new_or_load(desc, chng, db, Network::Bitcoin).expect_err("wrong network");
        assert!(
            matches!(
                err,
    ...
    // wrong genesis hash
    {
        ...
        let db =
            bdk_file_store::Store::open_or_create_new(DB_MAGIC, &file_path).expect("must open db");
        let err =
            Wallet::new_or_load_with_genesis_hash(desc, chng, db, Network::Testnet, exp_blockhash)
    ...
    // non unique descriptors
    {
        ...
        let err = Wallet::new_or_load(desc, desc, db, Network::Testnet)
            .expect_err("non unique descriptors");
        assert!(matches!(
            err,
            NewOrLoadError::Descriptor(DescriptorError::NonUnique(d)) if d == descriptor,
        ))
    }
    ...
    // all parameters match
    {
        let db =
            bdk_file_store::Store::open_or_create_new(DB_MAGIC, &file_path).expect("must open db");
        let wallet =
            Wallet::new_or_load(desc, chng, db, Network::Testnet).expect("must recover wallet");
        assert_eq!(wallet.network(), Network::Testnet);
...
fn test_create_tx_policy_path_no_csv() {
    let (desc, chng) = get_test_wpkh_with_change();
    let mut wallet = Wallet::new_no_persist(desc, chng, Network::Regtest).unwrap();
...
fn test_include_output_redeem_witness_script() {
    ...
    let chng = "sh(wsh(multi(1,cVpPVruEDdmutPzisEsYvtST1usBR3ntr8pXSyt6D2YYqXRyPcFW,cRjo6jqfVNP33HhSS76UhXETZsGTZYx8FMFvR9kpbtCSV1PmdZdu)))";
...
fn test_unused_address() {
    ...
    let chng = get_test_wpkh();
    let mut wallet = Wallet::new_no_persist(desc, chng, Network::Testnet).unwrap();
