    ...
    L: fmt::Display,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            NewOrLoadError::Backend(e) => write!(f, "failed to load from persistence: {}", e),
...
impl<D> Wallet<D> {
        ...
        D: PersistBackend<ChangeSet>,
    {
        let changeset = db
            .load_from_persistence()
            .map_err(NewOrLoadError::Backend)?;
        match changeset {
            Some(changeset) => {
                let wallet = Self::load_from_changeset(db, changeset).map_err(|e| match e {
                    LoadError::Backend(e) => NewOrLoadError::Backend(e),
