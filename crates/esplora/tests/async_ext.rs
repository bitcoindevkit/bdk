...
use bdk_core::bitcoin::consensus::encode::{deserialize_hex, serialize_hex};
use bdk_esplora::EsploraAsyncExt;
use bdk_testenv::corepc_client::client_sync::Input;
...
pub async fn detect_receive_tx_cancel() -> anyhow::Result<()> {
    ...
    let send_tx_outputs = BTreeMap::from([(
        receiver_addr.to_string(),
        (selected_utxo.amount.to_unsigned()? - SEND_TX_FEE).to_btc(),
    )]);
    let send_tx = rpc_client
        ...
        .0;
    let send_tx = rpc_client
        .sign_raw_transaction_with_wallet(&serialize_hex(&send_tx))?
        .into_model()?
        .raw_transaction;

    // Create and sign the `undo_send_tx` transaction. This redirects funds back to the sender
    // address.
    let undo_send_outputs = BTreeMap::from([(
        sender_addr.to_string(),
        (selected_utxo.amount.to_unsigned()? - UNDO_SEND_TX_FEE).to_btc(),
    )]);
    let undo_send_tx = rpc_client
        ...
        .0;
    let undo_send_tx = rpc_client
        .sign_raw_transaction_with_wallet(&serialize_hex(&undo_send_tx))?
        .into_model()?
        .raw_transaction;
