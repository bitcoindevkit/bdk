    ...
    I::IntoIter: Send,
{
    ...
    let mut tx_graph = fetch_txs_with_txids(
        client,
        // NOTE: @evanlinjin We need to collect here otherwise we run into lifetime issues, but I
        // do not know why. If someone can figure this out, that will be amazing.
        outpoints.iter().map(|op| op.txid).collect::<Vec<_>>(),
        parallel_requests,
    )
    .await?;
