...
impl CoinSelectorOpt {
    ...
    /// Generate [`CoinSelectorOpt`] from `txout`s.
    pub fn fund_outputs(
        txouts: &[TxOut],
        drain_output: &TxOut,
...
impl<'a> CoinSelector<'a> {
    ...
    /// Deselect candidate at index.
    pub fn deselect(&mut self, index: usize) -> bool {
        self.selected.remove(&index)
    }

    /// Whether candidate at index is selected.
    pub fn is_selected(&self, index: usize) -> bool {
        self.selected.contains(&index)
    }

    /// Returns true when there is nothing selected.
    ...
    /// Iterates unselected candidates.
    ...
    /// Iterates indexes of selected candidates.
    pub fn selected_indexes(&self) -> impl Iterator<Item = usize> + '_ {
        self.selected.iter().cloned()
    }

    /// Iterates indexes of unselected candidates.
    ...
    /// Returns true when all input candidates are selected.
    ...
    /// Attempt to select until we can call `finish` successfully.
    ...
    /// Succeeds when parameters are all satisfied and we have a valid selection.
...
/// Represents a selection failure.
#[derive(Clone, Debug)]
...
impl core::fmt::Display for SelectionFailure {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(
            ...
            self.selected, self.missing, self.constraint
        )
    }
}
...
/// Represents a constraint that is not met for coin selection.
...
pub struct Selection {
    ...
    /// Excess value (in satoshis).
    pub excess: u64,
    /// Avaliable excess strategies.
    pub excess_strategies: HashMap<ExcessStrategyKind, ExcessStrategy>,
}

/// The kind of excess strategy.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, core::hash::Hash)]
pub enum ExcessStrategyKind {
    /// Excess goes to fee.
    ToFee,
    /// Excess goes to recipient.
    ToRecipient,
    /// Excess goes to drain output.
    ToDrain,
}

/// Strategy of dealing with excess.
...
impl Selection {
    /// Apply selection to original candidates.
...
#[cfg(test)]
mod test {
    use crate::bdk_core::coin_select::{ExcessStrategyKind, SelectionConstraint};
