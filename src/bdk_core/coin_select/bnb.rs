...
impl BranchStrategy {
    /// Returns true if branch strategy is one that continues (does not backtrack).
...
pub struct Bnb<'c, S> {
    ...
    /// Current position within candidate pool.
    pub pool_pos: usize,
    /// Current best score (we want to minimize this).
    pub best_score: S,

    /// Current selection.
    pub selection: CoinSelector<'c>,
    /// Remaining absolute value of candidate pool.
    pub rem_abs: u64,
    /// Remaining effective value of candidate pool.
    pub rem_eff: i64,
}
...
/// An iterator for branch and bound.
...
mod test {
    ...
    use crate::bdk_core::coin_select::evaluate_cs::evaluate;
    use crate::bdk_core::coin_select::ExcessStrategyKind;

    use super::{
        coin_select_bnb,
        evaluate_cs::{Evaluation, EvaluationFailure},
        tester::Tester,
        CoinSelector, CoinSelectorOpt, WeightedValue,
    };
        ...
        for (opts, expect_solution, expect_selected) in test_cases {
            ...
            match res {
                Ok(eval) => {
                    ...
                    assert_eq!(eval.solution.selected.len(), expect_selected as usize);
                }
